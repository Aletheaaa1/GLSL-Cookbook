...
#pragma region Configue
	std::string shader_dir = "./Shaders/PointSprite/";
	Shader shader{ shader_dir + "point_sprite.vs", shader_dir + "point_sprite.fs", shader_dir + "point_sprite.gs" };
	Texture texture{ "./Textures/box.png" };

	constexpr unsigned int SPRITE_NUMBER = 50;
	std::unique_ptr<float[]> locations = std::make_unique<float[]>(3 * SPRITE_NUMBER);

	std::uniform_real_distribution<float> random_float(0.0f, 1.0f);
	std::default_random_engine random_generator;

	for (unsigned int i = 0; i < SPRITE_NUMBER; i++)
	{
		locations[3 * i] = random_float(random_generator) * 2.0f - 1.0f;
		locations[3 * i + 1] = random_float(random_generator) * 2.0f - 1.0f;
		locations[3 * i + 2] = random_float(random_generator) * 2.0f - 1.0f;
	}
	VertexBufferObject vbo{ locations.get(), sizeof(float) * SPRITE_NUMBER * 3 };
	vbo.Bind();

	VertexArrayObject vao;
	vao.Bind();
	vao.AddBuffer(0, 3, GL_FLOAT, false, 3 * sizeof(float), 0);
...
#pragma region Render
		...
		shader.SetUniform1f("halfSize", 0.15);

		glPointSize(50);
		glDrawArrays(GL_POINTS, 0, SPRITE_NUMBER);
